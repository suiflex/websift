...
## 9. Worker path is only valid from a source checkout

**Gap.** `WEBSIFT_WORKER_ARGS` defaults to `browser-worker/src/main.ts`, a path relative to the
working directory. That resolves in a source checkout and fails for an installed binary launched
from anywhere else, so worker extraction is effectively source-only today. Browser rendering is
already tracked as a gap, but this makes even the shipped worker mode unreachable after install.

**Done when.** The worker entry point is resolved against the installation directory or shipped
beside the binary, and `websift doctor` reports whether it was found.
